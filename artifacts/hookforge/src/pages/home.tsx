...
import { Loader2, Sparkles, Zap, Lock, RefreshCw, AlertCircle } from "lucide-react";
...
export default function Home() {
  ...
  const generateMutation = useGenerateScript({
    mutation: {
      onSuccess: (data: any) => {
        // Assuming data is of type GeneratedScript
        setScript({
          hook: data.hook || "This hook will grab attention immediately.",
          body: data.body || "This is the core value of the short.",
          cta: data.cta || "Like and subscribe for more.",
        });
        
        // Handle custom response fields if they exist in the wrapper, otherwise we manage locally
        const newRemaining = data.remainingRequests !== undefined ? data.remainingRequests : Math.max(0, remainingRequests - 1);
        setRemainingRequests(newRemaining);
        if (newRemaining === 0 || data.isLimitReached) {
      ...
      onError: (error: any) => {
        ...
        // Check for 429 or FREE_LIMIT_REACHED
        if (error.status === 429 || error?.data?.error === "FREE_LIMIT_REACHED") {
          setIsLimitReached(true);
          setRemainingRequests(0);
          toast({
            title: "Limit Reached",
            description: "You've used all your free scripts for today.",
            variant: "destructive",
          });
          return;
        }
        ...
        toast({
          ...
          description: error.message || "Failed to forge your script. Please try again.",
          variant: "destructive",
        });
      }
    }
  ...
  const handleGenerate = (e: React.FormEvent) => {
    ...
    if (isLimitReached || remainingRequests <= 0) {
      toast({
        title: "Upgrade Required",
        description: "Please unlock unlimited to generate more scripts.",
        variant: "default",
      });
      return;
    }
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-background text-foreground flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8 selection:bg-primary/30">
      ...
      {/* Header / Hero */}
      <div className="w-full max-w-3xl flex flex-col items-center text-center mb-12 animate-fade-in-up">
        ...
        <p className="text-muted-foreground text-lg md:text-xl max-w-xl mx-auto font-medium">
          Stop guessing. Generate high-retention YouTube Shorts scripts engineered for maximum watch time.
        </p>
      </div>
      ...
      <div className="w-full max-w-2xl mb-8 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
        <form onSubmit={handleGenerate} className="relative group">
          <div className="absolute -inset-0.5 bg-gradient-to-r from-primary/50 to-primary/0 rounded-xl blur opacity-30 group-hover:opacity-60 transition duration-500"></div>
      ...
      {script && (
        <div className="w-full max-w-3xl flex flex-col gap-6 animate-fade-in-up">
          ...
          <div className="grid gap-6">
            {/* HOOK */}
            <Card className="bg-card border-border overflow-hidden relative border-l-4 border-l-primary shadow-lg group hover:border-border transition-colors">
              ...
              <CardContent className="p-6 sm:p-8 pt-10 relative">
                ...
                <p className="text-2xl sm:text-3xl font-black text-white leading-tight mt-4">
                  {script.hook}
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-card border-border overflow-hidden relative shadow-md">
              <CardContent className="p-6 sm:p-8 pt-10 relative">
                ...
                <p className="text-lg text-card-foreground leading-relaxed mt-4 whitespace-pre-wrap font-medium">
                  {script.body}
                </p>
              </CardContent>
            </Card>
          ...
          <div className="rounded-2xl p-1 bg-gradient-to-br from-primary/40 via-primary/10 to-transparent">
            <div className="bg-card rounded-xl p-8 text-center flex flex-col items-center">
              ...
              <p className="text-muted-foreground mb-6 font-medium max-w-md">
                You've hit the daily limit for free viral scripts. Upgrade to Pro to get unlimited access and crush the algorithm.
              </p>
              <Button size="lg" className="w-full sm:w-auto px-8 font-bold text-base" asChild>
                <a href="#upgrade">Unlock Unlimited — $19/mo</a>
              </Button>
